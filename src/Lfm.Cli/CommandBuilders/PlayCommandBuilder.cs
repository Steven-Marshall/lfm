...
public static class PlayCommandBuilder
{
    ...
    {
        var command = new Command("play", "Play or queue a track or album on Spotify");
        ...
        var deviceOption = new Option<string?>(
            ...
            description: "Spotify device to use (overrides config default)");
        ...
        // Handler
        command.SetHandler(async (string artist, string? track, string? album, string? device, bool queue, bool json) =>
        {
            var playCommand = services.GetRequiredService<PlayCommand>();
            var result = await playCommand.ExecuteAsync(artist, track, album, device, queue, json);
            Environment.ExitCode = result;

        }, artistArgument, trackOption, albumOption, deviceOption, queueOption, jsonOption);
